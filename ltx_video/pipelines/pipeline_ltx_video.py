...
from ltx_video.models.autoencoders.motion_decoder import ElasticMotionDecoder
...
class LTXVideoPipeline(DiffusionPipeline):
    ...
    def __call__(
            ...
            save_latent_path: Optional[str] = None,  # 新增：保存latent的路径
            ...
            # 运动推理模式参数
            motion_mode: bool = False,  # 新增：运动模式开关
            motion_channels_per_person: int = 69,  # 新增：每人运动参数通道数
            motion_target_frames: Optional[int] = None,  # 新增：目标帧数（运动模式必需）
            save_motion_params_path: Optional[str] = None,  # 新增：保存运动参数路径
            **kwargs,
        ...
        Args:
            ...
            save_latent_path (`str`, *optional*):
                保存生成的latent到指定路径，如果为None则不保存
            ...
            motion_mode (`bool`, *optional*, defaults to `False`):
                是否启用运动推理模式。启用后，输出为运动参数而非图像/视频。
            motion_channels_per_person (`int`, *optional*, defaults to `79`):
                每人运动参数通道数。
            motion_target_frames (`int`, *optional*):
                目标帧数（运动模式必需）。如果不指定，则根据时间下采样因子自动计算。
            save_motion_params_path (`str`, *optional*):
                保存运动参数到指定路径。
        ...
        if motion_mode:
            return self.motion_inference(
                ...
                save_latent_path=save_latent_path,
                ...
                save_motion_params_path=save_motion_params_path,
                **kwargs
            )
    ...
    def motion_inference(
            ...
            output_type: Optional[str] = "pil",
            return_dict: bool = True,
            save_latent_path: Optional[str] = None,
            callback_on_step_end: Optional[Callable[[int, int, Dict], None]] = None,
            conditioning_items: Optional[List[ConditioningItem]] = None,
            decode_timestep: Union[List[float], float] = 0.0,
            decode_noise_scale: Optional[List[float]] = None,
            ...
            motion_target_frames: Optional[int] = None,
            save_motion_params_path: Optional[str] = None,
            **kwargs,
    ) -> Union[MotionVAEOutput, Tuple]:
        """
        运动推理模式
        ...
        """
        logger.info("=" * 60)
        logger.info("启动运动推理模式")
        logger.info("=" * 60)

        # 检查VAE类型
        is_motion_decoder_only = hasattr(self.vae, 'is_motion_vae') and self.vae.is_motion_vae

        if not isinstance(self.vae, MotionCausalVideoAutoencoder) and not is_motion_decoder_only:
            logger.warning("当前VAE不是MotionCausalVideoAutoencoder或MotionDecoderOnly类型")
            if isinstance(self.vae, CausalVideoAutoencoder):
                # 创建运动VAE
                motion_vae = MotionCausalVideoAutoencoder(
                    base_vae=self.vae,
                    motion_channels_per_person=motion_channels_per_person
                )
                self.vae = motion_vae
                logger.info("已成功转换为MotionCausalVideoAutoencoder")
            elif isinstance(self.vae, ElasticMotionDecoder):
                logger.info("当前VAE是MotionDecoder类型，直接使用")
            else:
                raise ValueError(
                    f"运动推理模式需要MotionCausalVideoAutoencoder、MotionDecoder或CausalVideoAutoencoder，"
                    f"当前类型为: {type(self.vae)}"
                )

        # 验证运动目标帧数 - 修复这里的错误计算
        if motion_target_frames is None:
            # 重要修复：应该使用与视频生成相同的时间下采样逻辑
            # 从vae获取正确的时间下采样因子
            if hasattr(self.vae, 'temporal_downscale_factor'):
                # 与original_inference中相同的计算逻辑
                latent_num_frames = num_frames // self.video_scale_factor
                if isinstance(self.vae, (CausalVideoAutoencoder, MotionCausalVideoAutoencoder, ElasticMotionDecoder)):
                    latent_num_frames += 1
                motion_target_frames = latent_num_frames * self.vae.temporal_downscale_factor
                logger.info(f"自动计算目标帧数: {motion_target_frames} "
                            f"(输入帧数: {num_frames}, 时间下采样因子: {self.vae.temporal_downscale_factor})")
            else:
                # 使用默认的视频缩放因子
                motion_target_frames = num_frames * self.video_scale_factor
                logger.info(f"自动计算目标帧数: {motion_target_frames} "
                            f"(输入帧数: {num_frames}, 视频缩放因子: {self.video_scale_factor})")

        # 检查必要参数
        assert motion_target_frames > 0, "motion_target_frames必须大于0"
        assert motion_channels_per_person > 0, "motion_channels_per_person必须大于0"

        logger.info(f"运动推理配置:")
        logger.info(f"  - 目标帧数: {motion_target_frames}")
        logger.info(f"  - 每人运动通道数: {motion_channels_per_person}")
        logger.info(f"  - 输入分辨率: {height}x{width}")
        logger.info(f"  - 输入帧数: {num_frames}")
        logger.info(f"  - 视频缩放因子: {self.video_scale_factor}")
        logger.info(f"  - VAE缩放因子: {self.vae_scale_factor}")

        # 修改输出类型为latent，因为我们需要latents进行运动解码
        original_output_type = output_type
        output_type = "latent"

        # 关键修复：确保使用与视频生成完全相同的参数调用original_inference
        try:
            result = self.original_inference(
                height=height,
                width=width,
                num_frames=num_frames,
                frame_rate=frame_rate,
                ...
                media_items=media_items,
                tone_map_compression_ratio=tone_map_compression_ratio,
                # 关键：确保is_video=True，这样video_scale_factor才会生效
                is_video=True,
            ...
            # 提取latents
            if return_dict:
                generated_latents = result.images
            else:
                generated_latents = result[0]

            logger.info(f"生成的latents形状: {generated_latents.shape}")



            # 解码为运动参数
            logger.info("开始解码latents为运动参数...")
            motion_params = vae_decode_motion(
                latents=generated_latents,
                vae=self.vae,
                target_frames=motion_target_frames,
                split_size=1,
                vae_per_channel_normalize=kwargs.get("vae_per_channel_normalize", True),
                timestep=decode_timestep if decode_timestep else None,
            )

            logger.info(f"解码后的运动参数形状: {motion_params.shape}")
            ...
            motion_output = MotionVAEOutput(
                ...
                metadata={
                    ...
                    'motion_target_frames': motion_target_frames,
                    'motion_channels_per_person': motion_channels_per_person,
                    'num_persons': motion_params.shape[4],  # 宽度维度代表人
                    'height': height,
                    'width': width,
                    'num_frames': num_frames,
                    'frame_rate': frame_rate,
            ...
            # 保存运动参数（如果指定了路径）
            if save_motion_params_path:
                save_motion_params(
                    motion_output=motion_output,
                    filepath=save_motion_params_path,
                    format="pt"  # 默认保存为PyTorch格式
                )
                logger.info(f"运动参数已保存到: {save_motion_params_path}")

            # 按人分割运动参数（可选）
            persons_motion = motion_output.split_by_person()
            logger.info(f"运动参数已按{len(persons_motion)}人分割")
            for i, person_motion in enumerate(persons_motion):
                logger.info(f"  第{i + 1}人运动形状: {person_motion.shape}")
    ...
    def original_inference(
        ...
        这是原有的__call__方法逻辑，现在拆分为独立的方法
        """
        # 关键修复：确保is_video参数正确传递
        is_video = kwargs.get("is_video", False)

        # 确保video_scale_factor正确设置
        ...
        # ===========================================
        print("=" * 80)
        print("🔍 DEBUG: VAE配置信息 (original_inference开始)")
        print("=" * 80)

        # 1. 打印VAE类型和关键属性
        print(f"VAE类型: {type(self.vae)}")
        print(f"VAE类名: {self.vae.__class__.__name__}")

        # 2. 尝试获取spatial_downscale_factor
        if hasattr(self.vae, 'spatial_downscale_factor'):
            print(f"vae.spatial_downscale_factor: {self.vae.spatial_downscale_factor}")
        elif hasattr(self.vae.decoder, 'spatial_downscale_factor'):
            print(f"vae.decoder.spatial_downscale_factor: {self.vae.decoder.spatial_downscale_factor}")
        else:
            # 尝试计算
            try:
                # 从原始VAE配置推导
                if hasattr(self.vae, 'encoder') and hasattr(self.vae.encoder, 'down_blocks'):
                    # CausalVideoAutoencoder类型
                    down_blocks = self.vae.encoder.down_blocks
                    spatial_factor = 2 ** len(down_blocks)  # 每个下采样块通常下采样2倍
                    print(f"计算得到的spatial_downscale_factor: {spatial_factor}")
                else:
                    print("无法获取spatial_downscale_factor: 无down_blocks属性")
            except Exception as e:
                print(f"无法获取spatial_downscale_factor: {e}")

        # 3. 打印latent_channels
        if hasattr(self.vae, 'latent_channels'):
            print(f"vae.latent_channels: {self.vae.latent_channels}")
        elif hasattr(self.vae, 'config') and hasattr(self.vae.config, 'latent_channels'):
            print(f"vae.config.latent_channels: {self.vae.config.latent_channels}")
        else:
            print(f"无法获取latent_channels")

        # 4. 打印temporal_downscale_factor
        if hasattr(self.vae, 'temporal_downscale_factor'):
            print(f"vae.temporal_downscale_factor: {self.vae.temporal_downscale_factor}")
        elif hasattr(self.vae.decoder, 'temporal_downscale_factor'):
            print(f"vae.decoder.temporal_downscale_factor: {self.vae.decoder.temporal_downscale_factor}")
        else:
            print(f"无法获取temporal_downscale_factor")

        # 5. 打印缩放因子（这是最关键的计算）
        print(f"\n📏 缩放因子信息:")
        print(f"  self.vae_scale_factor: {self.vae_scale_factor}")
        print(f"  self.video_scale_factor: {self.video_scale_factor}")
        print(f"  is_video: {is_video}")

        # 6. 打印Transformer配置
        print(f"\n🤖 Transformer配置:")
        print(f"  Transformer.in_channels: {self.transformer.config.in_channels}")
        print(f"  Transformer类型: {type(self.transformer)}")

        # 7. 计算并打印latent形状
        latent_height = height // self.vae_scale_factor
        latent_width = width // self.vae_scale_factor
        latent_num_frames = (num_frames - 1) // self.video_scale_factor + 1  # 🔥 修改这里

        print(f"\n📐 Latent形状计算:")
        print(f"  输入分辨率: {height}x{width}x{num_frames}")
        print(f"  latent_height: {height} // {self.vae_scale_factor} = {latent_height}")
        print(f"  latent_width: {width} // {self.vae_scale_factor} = {latent_width}")
        print(f"  latent_frames: ({num_frames}-1) // {self.video_scale_factor} + 1 = {latent_num_frames}")

        print(
            f"  最终latent形状: (batch, {self.transformer.config.in_channels}, {latent_num_frames}, {latent_height}, {latent_width})")

        # 9. 检查是否存在is_motion_vae标记
        if hasattr(self.vae, 'is_motion_vae'):
            print(f"\n⚠️ VAE有is_motion_vae标记: {self.vae.is_motion_vae}")
            print(f"  运动模式特殊属性:")
            if hasattr(self.vae, 'motion_channels_per_person'):
                print(f"  motion_channels_per_person: {self.vae.motion_channels_per_person}")
        ...
        if hasattr(self.vae, 'config'):
            ...
            if isinstance(config, dict):
                for key, value in list(config.items())[:10]:  # 只打印前10个
                    print(f"  {key}: {value}")
            elif hasattr(config, '__dict__'):
                for key, value in list(vars(config).items())[:10]:
                    print(f"  {key}: {value}")
        ...
        print(f"  Transformer.in_channels: {self.transformer.config.in_channels}")
        print(f"  latent_num_frames: {latent_num_frames}")
        ...
        if save_latent_path is not None:
            # 确保保存目录存在
            os.makedirs(os.path.dirname(save_latent_path), exist_ok=True)
            # 保存latent
            torch.save(latents.cpu(), save_latent_path)
            logger.info(f"Latent已保存至: {save_latent_path}")
            print(f"生成的latent形状: {latents.shape}")
        # ======================================

        if output_type != "latent":
            if hasattr(self.vae.decoder, 'timestep_conditioning') and self.vae.decoder.timestep_conditioning:
    ...
    def prepare_conditioning(
        ...
        """
        is_motion_decoder = isinstance(self.vae, ElasticMotionDecoder)
        is_causal_vae = isinstance(self.vae, (CausalVideoAutoencoder, MotionCausalVideoAutoencoder))

        # 如果是MotionDecoder模式且没有条件项，快速返回
        if is_motion_decoder and (conditioning_items is None or len(conditioning_items) == 0):
            print("🔄 MotionDecoder模式：跳过条件处理")
...
class LTXMultiScalePipeline:
    ...
    def __init__(
        ...
        # 复用 VAE 图像处理器（用于第一阶段视频后处理）
        self.image_processor = VaeImageProcessor(vae_scale_factor=self.video_pipeline.vae_scale_factor)

    # 视频保存工具函数（支持保存第一阶段低分辨率视频）
    def _save_video(self, video_frames, save_path, frame_rate=24):
        """
        保存视频到指定路径
        :param video_frames: 视频帧张量或 PIL 图像序列，形状为 (b, c, f, h, w) 或列表[PIL.Image]
        :param save_path: 保存路径（如 "./first_stage_video.mp4"）
        :param frame_rate: 视频帧率（默认24，需与生成时的 frame_rate 一致）
        """
        # 创建保存目录（若不存在）
        save_dir = os.path.dirname(save_path)
        if save_dir and not os.path.exists(save_dir):
            os.makedirs(save_dir, exist_ok=True)  # 添加exist_ok=True避免错误

        # 处理输入格式：若为张量，转换为 PIL 图像序列
        if isinstance(video_frames, torch.Tensor):
            # 张量形状：(batch_size, channels, num_frames, height, width)
            batch_size, _, num_frames, height, width = video_frames.shape
            # 取第一个 batch 的帧（若 batch_size>1，可循环处理）
            frames = video_frames[0].permute(1, 2, 3, 0)  # 转换为 (f, h, w, c)
            frames = (frames.clamp(-1, 1) + 1) / 2 * 255  # 归一化到 0-255
            frames = frames.to(torch.uint8).cpu().numpy()  # 转换为 numpy 数组
        elif isinstance(video_frames, list) and all(isinstance(f, Image.Image) for f in video_frames):
            # 若为 PIL 图像列表，直接转换为 numpy
            frames = [np.array(frame) for frame in video_frames]
        else:
            raise ValueError("不支持的视频帧格式，仅支持 torch.Tensor 或 PIL.Image 列表")

        # 用 imageio 写视频（需安装 imageio-ffmpeg：pip install imageio-ffmpeg）
        writer = imageio.get_writer(save_path, fps=frame_rate)
        for frame in frames:
            writer.append_data(frame)
        writer.close()
        print(f"✅ 视频已保存至：{save_path}")

    # 第一阶段 latent 解码为视频（复用原有 VAE 解码逻辑）
    def _decode_first_stage_latents(self, latents, downscaled_width, downscaled_height, frame_rate):
        """
        将第一阶段的低分辨率 latent 解码为可保存的视频
        :param latents: 第一阶段生成的 latent，形状为 (b, c, f, h_latent, w_latent)
        :param downscaled_width: 第一阶段低分辨率视频的宽度
        :param downscaled_height: 第一阶段低分辨率视频的高度
        :param frame_rate: 视频帧率
        :return: 解码后的视频帧（torch.Tensor，形状 (b, c, f, h, w)）
        """
        # 1. 检查 VAE 是否需要时间步条件（兼容原有逻辑）
        if hasattr(self.vae.decoder, 'timestep_conditioning') and self.vae.decoder.timestep_conditioning:
            noise = torch.randn_like(latents)
            decode_timestep = torch.tensor([0.0] * latents.shape[0], device=latents.device)  # 第一阶段解码用 timestep=0
            decode_noise_scale = torch.tensor([0.0] * latents.shape[0], device=latents.device)[:, None, None, None,
            None]
            latents = latents * (1 - decode_noise_scale) + noise * decode_noise_scale
        else:
            decode_timestep = None

        # 2. VAE 解码 latent 为低分辨率视频（复用原有 vae_decode 函数）
        low_res_video = vae_decode(
            latents=latents,
            vae=self.vae,
            is_video=True,  # 标记为视频（多帧）
            vae_per_channel_normalize=True,
            timestep=decode_timestep,
        )

        # 3. 图像后处理（归一化到 0-255，转换为标准格式）
        low_res_video = self.image_processor.postprocess(
            low_res_video,
            output_type="pt"  # 输出张量，便于后续保存
        )

        # 4. 确保视频尺寸匹配第一阶段的低分辨率（防止解码偏差）
        if low_res_video.shape[-2:] != (downscaled_height, downscaled_width):
            n_frames = low_res_video.shape[2]
            # 调整尺寸（复用原有插值逻辑）
            low_res_video = rearrange(low_res_video, "b c f h w -> (b f) c h w")
            low_res_video = F.interpolate(
                low_res_video,
                size=(downscaled_height, downscaled_width),
                mode="bilinear",
                align_corners=False
            )
            low_res_video = rearrange(low_res_video, "(b f) c h w -> b c f h w", f=n_frames)
    ...
    def __call__(
            ...
            first_pass: dict,
            second_pass: dict,
            save_first_stage_video: bool = True,  # 是否保存第一阶段视频
            final_video_path: str = "./final_video.mp4",  # 最终视频的保存路径
            first_stage_filename: str = "first_stage_low_res_video.mp4",  # 第一阶段视频的文件名
            *args: Any,
            **kwargs: Any,
    ) -> Any:
        ...
        frame_rate = kwargs.get("frame_rate", 24)  # 从输入参数获取帧率（默认24）
        ...
        # 构造第一阶段视频的保存路径（与最终视频同目录）
        first_stage_save_path = os.path.join(final_video_dir, first_stage_filename)
        ...
        if result.images is None:
            print("⚠️ 第一阶段低分辨率生成结果为 None，无法保存第一阶段视频！")
            save_first_stage_video = False
        else:
            print(f"✅ 第一阶段低分辨率生成结果正常，形状: {result.images.shape}，数据类型: {result.images.dtype}")
            latents = result.images

            # 若开启保存，解码并保存第一阶段视频
            if save_first_stage_video:
                # 解码 latent 为低分辨率视频
                first_stage_video = self._decode_first_stage_latents(
                    latents=latents,
                ...
                # 保存第一阶段视频到最终视频所在目录
                self._save_video(
                    video_frames=first_stage_video,
                    save_path=first_stage_save_path,
                    frame_rate=frame_rate
                )
        ...
        # 原有流程：上采样 + 第二阶段优化
        # --------------------------
        if save_first_stage_video and result.images is not None:
            # 上采样latents
            ...
            # 若第一阶段失败，沿用原有逻辑（避免流程中断）
            upsampled_latents = self._upsample_latents(self.latent_upsampler, result.images)
            upsampled_latents = adain_filter_latent(latents=upsampled_latents,
                                                    reference_latents=result.images)
        ...
        # 第二阶段输出检测（上采样后）
        if result.images is None:
            print("⚠️ 第二阶段高分辨率优化结果为 None！")
        else:
            print(f"✅ 第二阶段高分辨率优化结果正常，形状: {result.images.shape}，数据类型: {result.images.dtype}")
            if isinstance(result.images, torch.Tensor):
                has_nan = torch.isnan(result.images).any().item()
                has_inf = torch.isinf(result.images).any().item()
                print(f"   包含 NaN: {has_nan}, 包含无穷大: {has_inf}")
        ...
        if original_output_type != "latent":
            ...
            result.images = rearrange(videos, "(b f) c h w -> b c f h w", f=num_frames)
            print(f"✅ 最终缩放后结果正常，形状: {result.images.shape}，数据类型: {result.images.dtype}")
